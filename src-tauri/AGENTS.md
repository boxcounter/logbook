...
├── error_log.rs     // init, log_info/log_error, log_frontend_info/log_frontend_error, log_command_enter/log_command_exit, install_panic_hook
...
- **单元测试**：`src/` 内 `#[cfg(test)] mod tests`。模块内函数级行为，milliseconds 级。可直接操作文件系统——用 `std::env::temp_dir()` 下独立临时目录、事后清理；但必须并行安全（cargo test 默认并行）：触碰进程内全局态的测试（如 `error_log` 的 `LOG_PATH`）必须用模块内互斥锁串行化
