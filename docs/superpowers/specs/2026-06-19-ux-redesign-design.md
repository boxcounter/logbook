...
**字号 scale**: `--app-text-2xs`(9) → `--app-text-xs-alt`(11) → `--app-text-micro`(10) → `--app-text-xs`(12) → `--app-text-sm`(13) → `--app-text-base`(14) → `--app-text-lg`(18) → `--app-text-xl`(20)
...
| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| 月份名 | `--app-text-base` | `--weight-bold` | `--color-text-primary` | — |
| 年份 | `--app-text-xs-alt` | 400 | `--color-text-secondary` | — |
| 星期头 M-S | `--app-text-2xs` | 400 | `--color-text-secondary` | system-ui |
| 日期数字 | `--app-text-micro` | 400→700(heavy) | 按色阶 | `--app-font-mono` |
...
| 月份总计 | `--app-text-xs-alt` | `--weight-semibold` | `--color-text-primary` | `/ month` 10px 400 `--color-text-secondary` |
| 导航箭头 | `--app-text-xs` | 400 | `--color-text-secondary` | hover → `--color-text-primary` |

### 4.2 QuickJumpPopover (Sidebar 顶部，点击月份标签弹出)

| 元素 | 字号 | 颜色 | 特殊 |
|------|------|------|------|
| Year select | `--app-text-xs` | `--color-text-primary` | border: `--color-border-form`, `--radius-form-lg` |
| Month select | `--app-text-xs` | `--color-text-primary` | 同上 |
| Go 提示 | `--app-text-2xs` | `--color-text-secondary` | — |
...
| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| Section label | `--app-text-micro` | `--weight-bold` | `--color-text-secondary` | uppercase, letter-spacing 0.5px |
| Role name | `--app-text-xs` | `--weight-semibold` | `--color-text-primary` | ▸ 折叠 / ▾ 展开 |
| Spent/allocation | `--app-text-xs-alt` | `--weight-semibold` | `--color-text-primary` | spent `--app-font-mono`, allocation 400 `--color-text-secondary` |
| Progress bar | 高 4px | — | bg: `--color-divider`, fill: brand gradient | `--radius-sm` |
| Goal name | `--app-text-xs-alt` | 400 | `--color-text-secondary` | `max-width:130px`, 超出省略，hover tooltip 全文 |
| Goal spent | `--app-text-xs-alt` | `--weight-medium` | `--color-text-primary` | `--app-font-mono`. 0→`--color-text-secondary` |
...
| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| Day title | `--app-text-xl` | `--weight-bold` | `--color-text-primary` | letter-spacing: -0.3px |
| Today badge | `--app-text-micro` | `--weight-semibold` | `--color-brand-link` | bg: `--color-brand-soft-bg`, radius 4px |
| Day summary | `--app-text-xs` | 400 | `--color-text-secondary` | entry count `--app-font-mono`, total `--app-font-mono` |
...
| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| Item text | `--app-text-base` | `--weight-medium` | `--color-text-primary` | 最多 2 行，超出省略，hover tooltip 全文 |
| Duration | `--app-text-sm` | 400 | `--color-text-primary` | `--app-font-mono`, `tabular-nums`, ml 16px |
| Chip (cat) | `--app-text-micro` | 450 | `--color-chip-cat-text` | bg: `--color-chip-cat-bg`, max-width:100px, 省略 |
| Chip (biz) | `--app-text-micro` | 450 | `--color-chip-biz-text` | bg: `--color-chip-biz-bg`, 同上 |
| Chip (imp) | `--app-text-micro` | 450 | `--color-chip-imp-text` | bg: `--color-chip-imp-bg`, 同上 |
| Chip (goal) | `--app-text-micro` | 450 | `--color-chip-goal-text` | bg: `--color-chip-goal-bg`, 同上 |
| Edit trigger ⋯ | `--app-text-base` | 400 | `--color-text-secondary` | 默认 opacity:0, row hover→1, icon hover→`--color-brand-solid` |
| Row hover | — | — | bg: `--color-surface-muted` | border: `--color-divider` 出现 |

**状态**：
- **默认**：背景透明，border 透明，⋮ 不可见
- **Hover**：bg `--color-surface-muted`，border `--color-divider`，⋮ 出现
- **编辑中**：bg `--color-surface`，border `--color-brand-solid`，shadow `--shadow-focus-ring`
- **刚添加**：bg `--anim-highlight-bg`，border `--anim-highlight-border`，`--anim-highlight-duration` 动画消退

### 4.6 EntryRowEdit (编辑模式，内嵌在 EntryRow 中触发)

| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| Item input | `--app-text-base` | `--weight-medium` | `--color-text-primary` | border:none, 透明 bg, 全宽 |
| Duration input | `--app-text-sm` | 400 | `--color-text-primary` | `--app-font-mono`, width:56px, text-align:right, 1px border `--color-border-form`, focus→`--color-brand-solid` |
| Duration unit | `--app-text-xs-alt` | 400 | `--color-text-secondary` | "min" |
| Edit chip | `--app-text-micro` | 500 | 按类型 | × 可删除（opacity:0.5, hover→1） |
| +tag chip | `--app-text-micro` | 500 | `--color-text-secondary` | border:1px dashed `--color-border-form`, hover→`--color-text-secondary` |
| Save btn | `--app-text-micro` | `--weight-semibold` | #fff | bg: `--color-brand-solid`, `--radius-form` |
| Cancel btn | `--app-text-micro` | `--weight-semibold` | `--color-text-secondary` | 透明 bg, hover→`--color-text-secondary` |
| Delete btn | `--app-text-micro` | `--weight-semibold` | `--color-text-disabled` | 透明 bg, ml:auto, hover→`--color-danger` |

### 4.7 DayNote (Main 中，Entry List 和 Input 之间)

| 元素 | 字号 | 颜色 | 特殊 |
|------|------|------|------|
| Note text | `--app-text-xs` | `--color-text-secondary` | italic, cursor:text, hover bg→`--color-page-bg`, hover color→`--color-text-secondary` |

### 4.8 TwoLineInput (Main 底部)

| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| "+" prefix | `--app-text-lg` | 400 | `--color-brand-solid` | flex-shrink:0 |
| Item input | `--app-text-base` | 400 | `--color-text-primary` | border:none, 透明 bg, caret `--color-brand-solid` |
| Placeholder | `--app-text-base` | 400 | `--color-placeholder` | — |
| Enter badge | `--app-text-2xs` | `--weight-semibold` | `--color-text-secondary` | border `--color-border-form`, radius 4px, 默认 opacity:0.5, focused→1 |
| Input card | — | — | bg: `--color-surface`, border:2px `--color-border-form` | `--radius-card`, focused→border `--color-brand-solid` + `--shadow-focus-ring` |
| Token (cat) | `--app-text-micro` | `--weight-medium` | `--color-token-cat-text` | bg: `--color-token-cat-bg` |
| Token (biz) | `--app-text-micro` | `--weight-medium` | `--color-token-biz-text` | bg: `--color-token-biz-bg` |
| Token (imp) | `--app-text-micro` | `--weight-medium` | `--color-token-imp-text` | bg: `--color-token-imp-bg` |
| Token (goal) | `--app-text-micro` | `--weight-medium` | `--color-token-goal-text` | bg: `--color-token-goal-bg` |
| Token (dur) | `--app-text-micro` | `--weight-medium` | `--color-token-dur-text` | bg: `--color-token-dur-bg`, `--app-font-mono` |
| Token × | `--app-text-xs` | — | opacity:0.4 | hover→opacity:1 |
| Missing indicator | `--app-text-micro` | 450 | `--color-missing-text` | border:1.5px dashed `--color-missing-border`, 可点击 |
| Missing dot | 5×5px | — | `--color-missing-dot` | border-radius:50% |
| Hints row | `--app-text-micro` | 400 | `--color-text-disabled` | hover→`--color-text-secondary`, kbd 内 `--app-font-mono` |

### 4.9 DimensionPopover (输入 @ 后弹出)

| 元素 | 字号 | 字重 | 颜色 | 特殊 |
|------|------|------|------|------|
| Dim header | `--app-text-micro` | `--weight-bold` | `--color-popover-dim-header-text` | bg: `--color-popover-dim-header-bg`, uppercase, "DIM" 徽章 |
| Dim item | `--app-text-sm` | 400 | `--color-text-primary` | 左侧颜色条(width:3px, `--radius-sm`), hover bg:`--color-divider` |
| Dim item selected | `--app-text-sm` | `--weight-semibold` | `--color-brand-solid` | bg: `--color-popover-item-selected-bg` |
...
| Required meta | `--app-text-micro` | `--weight-medium` | `--color-warning` | — |
| Optional meta | `--app-text-micro` | 400 | `--color-text-disabled` | — |
| Val header | `--app-text-micro` | `--weight-bold` | `--color-popover-val-header-text` | bg: `--color-popover-val-header-bg`, ← back button |
| Val item | `--app-text-sm` | 400 | `--color-text-primary` | hover bg:`--color-divider` |
| Val item selected | `--app-text-sm` | `--weight-semibold` | `--color-brand-solid` | bg: `--color-popover-item-selected-bg` |
| Footer | `--app-text-2xs` | 400 | `--color-text-disabled` | `--app-font-mono` for keyboard hints |

### 4.10 FilePath (Main 底部，Input 下方)

| 元素 | 字号 | 颜色 | 特殊 |
|------|------|------|------|
| Path text | `--app-text-micro` | `--color-text-disabled` | text-align:right, cursor:pointer, 点击 open in editor |
...
- 新增 `--heatmap-*` 系列
- 字号 token 保持 `--app-text-*` 前缀、字体保持 `--app-font-*` 前缀。**不可改用 `--text-*` / `--font-*`**——Tailwind v4 将这两个命名空间用于 `@theme`（生成 `text-*` / `font-*` 工具类），裸用会冲突（见 commit `0acd2ee`）
